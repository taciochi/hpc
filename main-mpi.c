...
void stencil(float* inputvec, int m, int n, float* filtervec, int k, float* output, int b) {
    ...
    if (MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, output, input_size/size, MPI_FLOAT, MPI_COMM_WORLD) != MPI_SUCCESS) {
        fprintf(stderr, "Error: MPI_Allgather failed.\n");
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
}